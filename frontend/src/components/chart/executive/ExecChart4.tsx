...
function ExecChart4({ chart4 }: ExecChart4Props) {
  ...
  const enrichedData = useMemo(() => {
    ...
    return data.map((d, index) => {
      const normalized = maxSize === minSize ? 1 : (d.genreCount - minSize) / (maxSize - minSize);
      ...
      return {
        ...
        size: d.genreCount,
        fill: `rgb(${r}, ${g}, ${b})`,
        percentage: total > 0 ? (d.genreCount / total) * 100 : 0,
        rank: index + 1,
        total: data.length
      };
    });
  ...
  return (
    <div className='w-full h-full text-white mt-3 flex flex-col items-center'>
      <p className='text-center text-chart-title mb-1'>Genre Distribution</p>
